...
def generate_pk_clause(catalog_entry, state):
    ...
    if last_pk_fetched:
        pk_comparisons = ["({pk} > {last_pk_fetched})".format({'pk': common.escape(pk), 'last_pk_fetched': last_pk_fetched[pk]})
                          for pk in key_properties]
    else:
        pk_comparisons = ""

    sql = " WHERE {pk_comparisons} ORDER BY {escaped_columns} ASC".format({'pk_comparisons': " AND ".join(pk_comparisons),
                                                                           'escaped_columns': ", ".join(escaped_columns)})
