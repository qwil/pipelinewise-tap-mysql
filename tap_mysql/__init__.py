...
def binlog_stream_requires_historical(catalog_entry, state):
    ...
    # TODO: When would this pop up for binlog sync? It seems to never be set there.
...
def get_non_binlog_streams(mysql_conn, catalog, config, state):
    ...
    if currently_syncing:
        ...
        non_currently_syncing_streams = list(filter(lambda s: s.tap_stream_id != currently_syncing_stream, ordered_streams))

        # TODO: Would be nice to somehow assert this, the above logic is a bit brittle.
        # assert currently_syncing_stream not in non_currently_syncing_streams
...
def do_sync_full_table(mysql_conn, catalog_entry, state, columns):
    ...
    # TODO: Seem to be using this to skip re-syncing the table...
    state = singer.write_bookmark(state,
                                  catalog_entry.tap_stream_id,
